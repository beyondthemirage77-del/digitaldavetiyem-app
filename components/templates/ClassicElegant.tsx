...
const DEFAULT_BG =
  "linear-gradient(135deg, #2c1810, #4a2c1a, #2c1810)";
...
export function ClassicElegant({
  variant = "preview",
  brideName = "Ad",
  groomName = "Soyad",
  eventDate = "",
  ...
  if (mediaType === "video" && mediaUrls?.[0]) {
    ...
    const videoTextColor = textColor || "#2c1810";
  ...
  return (
    <div style={{ width: "390px", overflow: "hidden" }}>
      {/* Hero section */}
      <div
        style={{ position: "relative", minHeight: "844px" }}
      >
      <BackgroundLayer
        mediaType={mediaType}
        mediaUrls={mediaUrls}
      ...
      {/* Section 2 - Etkinlik Detayları (preview only - page renders with full functionality when live) */}
      {variant !== "live" && (
      ...
      {/* Section 3 - RSVP (preview only - page renders working form when live) */}
