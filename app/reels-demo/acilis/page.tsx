...
const ACILIS_PROPS = {
  templateId: 23,
  acilis_firmaAdi: "İrem Eczanesi",
  eventDate: "2026-11-11",
  eventTime: "14:00",
  venueName: "İrem Eczanesi",
  venueAddress: "Nişantaşı, İstanbul",
  mainTitle: "İrem Eczanesi",
  titleText: "Açılışımıza Davetlisiniz",
...
export default function ReelsDemoAcilisPage() {
  ...
  const handleRecord = useCallback(async () => {
    try {
      ...
      recorder.onstop = () => {
        ...
        a.href = url;
        a.download = `acilis-reels-${Date.now()}.${ext}`;
        a.click();
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center p-4 gap-6">
          ...
          <AnimatePresence mode="wait">
            {currentScene === "hook" && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-amber-950 to-slate-900"
              >
                <motion.p
              ...
              >
                ...
                >
                  <div style={{ transform: `scale(${W / 390})`, transformOrigin: "top center" }}>
                    ...
                    >
                      <TemplateRenderer variant="live" {...ACILIS_PROPS} />
                    </motion.div>
                  </div>
                </motion.div>
                <motion.div
              ...
              >
                ...
                >
                  ...
                  <div style={{ width: 390, transform: `scale(${W / 390})`, transformOrigin: "top center" }} className="flex-shrink-0">
                    <EventDetailsSection
                      eventDate={ACILIS_PROPS.eventDate}
                      eventTime={ACILIS_PROPS.eventTime}
                      venueName={ACILIS_PROPS.venueName}
                      venueAddress={ACILIS_PROPS.venueAddress}
                    />
                  </div>
                  <div style={{ width: 390, transform: `scale(${W / 390})`, transformOrigin: "top center" }} className="flex-shrink-0">
                    <RSVPSection />
                    ...
                    <div className="max-w-[85%] rounded-lg px-3 py-2" style={{ background: "#d9fdd3", boxShadow: "0 1px 2px rgba(0,0,0,0.1)" }}>
                      <div className="rounded-lg overflow-hidden border border-[#e0e0e0] bg-white mb-1">
                        <div className="relative w-full" style={{ aspectRatio: "9/16", minHeight: 120 }}>
                          <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-[#2c1810] to-[#1a0f0a] text-amber-100 px-4 py-2">
                            <p className="text-[8px] uppercase tracking-widest opacity-90">Açılışımıza Davetlisiniz</p>
                            <p className="text-[12px] font-bold mt-1">İrem Eczanesi</p>
                            <p className="text-[7px] mt-1 opacity-90">11 Kasım 2026</p>
                            <p className="text-[6px] mt-0.5 opacity-70">Nişantaşı, İstanbul</p>
                          </div>
                        </div>
                        <p className="text-[7px] text-[#666] px-2 py-1 text-center">digitaldavetiyem.com/irem-eczanesi</p>
                      </div>
            ...
            {currentScene === "cta" && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-amber-900 to-slate-900"
              >
                <motion.p
      ...
      <p className="text-xs text-white/50 text-center max-w-sm">
        Açılış şablonu · 9:16 Reels · Kaydet için butona tıkla, tarayıcıda &quot;Bu sekme&quot;yi seç.
      </p>
    </div>
  );
}
